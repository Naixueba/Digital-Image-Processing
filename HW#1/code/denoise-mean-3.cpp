...

{
    ...
    for(k=0;k<BytesPerPixel;k++){
        for(i=(N-1)/2;i<Size;i++){
            for(j=(N-1)/2;j<Size;j++){
                // do window filtering
                Imagedata[i][j][k] = 0.125*(imageExtend[i-1][j-1][k]+imageExtend[i-1][j][k]+imageExtend[i-1][j+1][k] +imageExtend[i][j-1][k]+imageExtend[i][j][k]+imageExtend[i][j+1][k] +imageExtend[i+1][j-1][k]+imageExtend[i+1][j][k]+imageExtend[i+1][j+1][k]);
            }
        }
    ...
    /////////////////////////// Some Questions ///////////////////////////
    // 1. how to use window matrix [1,1,1; 1,1,1; 1,1,1] to filter the image, rather than simply code the equation?
    //
    //////////////////////////// TO BE SOLVED ////////////////////////////
